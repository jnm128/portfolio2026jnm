...
const Hero: React.FC<HeroProps> = ({
  ...
  return <section className={cn('pt-28 pb-16 md:pt-36 md:pb-24 bg-background', className)}>
      <div className="container mx-auto px-4 md:px-8 max-w-6xl">
        {/* Hero Image Slideshow with Overlay Text */}
        <FadeIn>
          <div className="relative rounded-3xl overflow-hidden mb-16">
            {heroImages.map((image, index) => <img key={image} src={image} alt={`Hero slide ${index + 1}`} className={cn("w-full aspect-[4/3] object-cover transition-opacity duration-1000 absolute inset-0", index === currentIndex ? "opacity-100" : "opacity-0")} />)}
            <img src={heroImages[0]} alt="Hero placeholder" className="w-full aspect-[4/3] object-cover invisible" />
